...
// 有 systemInstruction 才補上頂層欄位（系統指令與逐場任務 prompt 分層）。
function withSystem(body, systemInstruction) {
  const s = (systemInstruction || "").trim();
  if (s) body.system_instruction = { parts: [{ text: s }] };
  return body;
}

// 走 inlineData：把 bytes base64 直接塞進請求。
async function generateInline(apiKey, model, prompt, base64, mimeType, systemInstruction) {
...
async function generateFromFile(apiKey, model, prompt, fileUri, mimeType, systemInstruction) {
  const url = `${BASE}/models/${model}:generateContent?key=${apiKey}`;
  const body = withSystem(
    {
 ...
 * @param {object} opts { apiKey, model, prompt, systemInstruction, onStatus, onRetry }
...
export async function generateMinutes(bytes, mimeType, { apiKey, model, prompt, systemInstruction, onStatus, onRetry } = {}) {
  ...
  return withRetry(
    async () => {
      if (bytes.byteLength <= INLINE_LIMIT) {
        ...
        return generateInline(apiKey, finalModel, finalPrompt, base64, mimeType, systemInstruction);
      }
      onStatus?.("上傳音訊中…");
      const uri = await uploadFile(apiKey, bytes, mimeType);
      onStatus?.("AI 整理中…");
      return generateFromFile(apiKey, finalModel, finalPrompt, uri, mimeType, systemInstruction);
    },
    { onRetry }
  );
}
