...
// 從貼上的連結／檔案 ID 取得影片資訊（供「貼連結」入口）。
// 含 size／時長供大檔提示，mimeType 供驗證，ownedByMe 供判斷輸出位置。
export async function getFileInfo(token, fileId) {
  const fields =
    "id,name,size,mimeType,ownedByMe,videoMediaMetadata(durationMillis)";
  const resp = await api(token, `${DRIVE}/files/${fileId}?fields=${q(fields)}`);
  return await resp.json();
}
